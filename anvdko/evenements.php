...
if(isset($_POST['updateProdCatPrincipale']))
{
    if (isset($_POST['id_categorie']) && is_array($_POST['id_categorie'])) {
        ...
        if (isset($_POST['ids_evenement']) && !empty($_POST['ids_evenement'])) {
            foreach (explode(',', $_POST['ids_evenement']) as $id_evenement) {
                $id_evenement = strip_tags(htmlspecialchars(trim($id_evenement)));

                $query = "UPDATE evenements 
                            SET id_cat_event= ".empty_to_NULL($id_categorie)." 
                            WHERE id = ".$id_evenement." AND id_configuration = '".$_SESSION["configuration"]["id"]."'";
...
{
    $id_evenement = strip_tags(htmlspecialchars(trim(crypt_decrypt_chaine($_POST["id_evenement"], 'D'))));
    $query = "DELETE FROM evenements WHERE id ='".$id_evenement."' AND id_configuration = '".$_SESSION["configuration"]["id"]."'";
  ...
  <body>
        ...
        <div class="mb-9">
          <div class="mx-n4 mx-lg-n6 mt-n5 position-relative mb-md-9" style="height:208px">
            ...
            <div class="faq-title-box position-relative bg-body-emphasis border border-translucent p-6 rounded-3 text-center mx-auto">
              <h3>Rechercher dans evenement</h3>
			  	...
			  	<div class="card card-fluid">
				    <div class="card-header border-0 p-1">
                        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">    
                             <a href="evenements_detail.php?ajouterevenement=" class="btn btn-primary btn-sm"><i class='fas fa-calendar-alt'></i> evenement</a>

                            <button id='btnCatevenement' class="btn btn-sm btn-phoenix-secondary" style='display: none;' data-bs-toggle="modal" data-bs-target="#ModalCatPrincipale">
                                <i class='fas fa-list'></i> Catégorie evenement
                            </button>
        ...
        <form action="evenements.php" method="post">
            ...
            ?>
                        ...
                        <div class="modal-footer">
                            <input type='hidden' name="ids_evenement">
    ...
    <script>
        $(document).ready(async function () {
            ...
            while (await listeevenements(nbreevenement)) {
                nbreevenement += 20;
            }

            $('#listeevenements #spinner').remove(); 
        });
    </script>
...
<script>
  function listeevenements(nbreevenement) {
      return new Promise(function(resolve, reject) {
          ...
          $.ajax({
              ...
              type: 'post',
              data: {
                  nbreevenement: nbreevenement, 
                  liste_evenements: ''
              },
              dataType: 'html',
              success: function (data) {
                  if (data.trim() !== '') {
                      $('#listeevenements').append(data);  // Affiche les données dans le tableau
                      resolve(true);
                  } else {
                      resolve(false);
                  }
...
<script>
    ...
    $('table tbody tr').each(function () {
        var dateActu = $(this).find('.date-evenement').text().trim();
...
function getSelectedCheckboxes()
{
    ...
    var ids_evenement = selectedIds.join(','); 

    if(ids_evenement.trim() == '')
    {
        $('#btnCatevenement').hide('fast'); 
    }
    else 
    {
        $('#btnCatevenement').show('fast'); 
    }

    $('#ModalCatPrincipale input[name="ids_evenement"]').val(ids_evenement); 
}
</script>
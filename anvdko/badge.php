...
if (!isset($_GET["id_membre"]) && !isset($id_membre)) {
    header("Location: index.php");
    exit;
}

$query = "SELECT *, DATE_FORMAT(date_naissance, '%d/%m/%Y') AS date_naissance FROM membres WHERE id = ".$id_membre;
$resultat = mysqli_query($bdd, $query) or die("Requête non conforme");

if (mysqli_num_rows($resultat) > 0) {
    $membre = mysqli_fetch_array($resultat);

    if (isset($membre['qr_url']) && !empty($membre['qr_url'])) {
        // Si l'URL de QR code existe déjà, l'utiliser
        $existingQrUrl = $membre['qr_url'];
        $qrUrl = $existingQrUrl;
    } else {
        // Sinon, générer un nouvel URL de QR code
        $qrUrl = "https://anvdko.rca-emergency.com/membres/voir_cotisation.php?id_membre=" . crypt_decrypt_chaine($membre['id'], 'C');
        
        $updateQuery = "UPDATE membres SET qr_url = '$qrUrl' WHERE id = ".$membre['id'];
...
<body>
  ...
  <div class="badge-container" id="badge-pdf">
    ...
    <div class="badge">
      ...
      <div class="info">
        <div><span>Nom :</span> <?php echo safe_safe_ucfirst($membre["nom"]);?> </div>
        <div><span>Prénoms :</span> <?php echo safe_safe_ucfirst($membre["prenom"]);?></div>
        <div><span>Né(e) le :</span> <?php echo $membre["date_naissance"];?></div>
        <div><span style="width: 120px;">Lieu résidence :</span> <?php echo safe_safe_ucfirst($membre["ville_commune"]);?> </div>
        <div><span style="width: 120px;">N° d'adhésion :</span> <?php echo $membre["num_adhesion"];?></div>
      </div>

      <div class="qr-section">
        <div class="qr-code" id="qrCode"></div>
        <div class="badge-type"><?php echo safe_safe_ucfirst($membre["poste_occupe"]);?></div>
      </div>
      ...
      <div class="footer">
        <div class="footer-left">
          <div><span>Contact :</span> <?php echo safe_safe_ucfirst($membre["num_telephone"]);?></div>
          <div style="visibility: hidden;"><span>Email :</span> <?php echo safe_safe_ucfirst($membre["email"]);?></div>
        </div>
  ...
  <<!-- Dans badge.php, remplacer les scripts par -->

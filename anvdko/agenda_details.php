...
{
    $_SESSION["id_agenda"] = strip_tags(htmlspecialchars(trim(crypt_decrypt_chaine($_GET["id_agenda"], 'D') )));
    reload_current_page();
}


if (isset($_GET["ajouterAgenda"])) {
    // Génère la référence de l'événement
    $evenement      = annee_ordre_referenceagenda($bdd, $_SESSION["configuration"]["id"]); 
    ...
    if (@mysqli_query($bdd, $query)) {
        $_SESSION["id_agenda"] = mysqli_insert_id($bdd); // récupère l'ID auto-incrémenté
        reload_current_page(); // recharge la page actuelle
    } 
}


if (isset($_POST["actualiserInfosagenda"])) {
    ...
    $espace_agenda = strip_tags(htmlspecialchars(trim($_POST["espace_agenda"])));
    $date_agenda = strip_tags(htmlspecialchars(trim($_POST["date_agenda"])));
    $heure_agenda = strip_tags(htmlspecialchars(trim($_POST["heure_agenda"])));
    ...
    if (!empty($_FILES['image_conferencie']['tmp_name'])) {
        ...
        if (file_put_contents($destination, $imageData)) {
            ...
            $query = "UPDATE agenda
                      SET nom = \"$nom\",
                          espace_agenda = \"$espace_agenda\",
                          date_agenda = \"$date_agenda\",
                          heure_agenda = \"$heure_agenda\",
...
$query = "SELECT *
           FROM agenda WHERE id_configuration = ".$_SESSION['configuration']['id']." AND id =".$_SESSION['id_agenda'];
$resultat = mysqli_query($bdd, $query) or die("Requête non conforme");  
$_SESSION['agenda'] = mysqli_fetch_array($resultat);
...
<html data-navigation-type="default" data-navbar-horizontal-shape="default" lang="fr-FR" dir="ltr">
  ...
  <head>
    ...
    <style type="text/css">  
        ...
        #divPrixagenda table tr:first-child .btnSuppr {visibility: hidden;}
    </style>
  </head>
                ...
                <nav class="mb-2" aria-label="breadcrumb">
                  <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item">
                        ...
                        <a href="agenda.php" class='text-secondary'>Retour</a>
                    </li>
                  </ol>
                </nav>
                <h4 class="mb-2">Référence agenda ( <span style='color: #2c2664;'> <?php echo safe_safe_ucfirst($_SESSION['agenda']['n_agenda']);?> </span> )</h4>
              </div>

              <div class="page-section">

                  <form id='formulaireagenda' method='post' action='agenda_details.php' onsubmit="prepareSubmit()" enctype="multipart/form-data">
                      <div class='row'>
                          <div class='col-md-8'>
                            ...
                            <input type="text" name="nom" class="form-control mb-1" value="<?php echo $_SESSION['agenda']['nom'];?>" required>
                            <label class="form-label">Titre</label>
                                            </div>                                       
                                        </div>
                                        <div class='col-12 mb-1'>
                                            <div class="form-floating">
                            <input type="text" name="espace_agenda" class="form-control mb-1" value="<?php echo $_SESSION['agenda']['espace_agenda'];?>" required>
                            <label class="form-label">Espace événement</label>
                                            </div>                                       
                                        </div>
                                        <div class='col-12 mb-1'>
                                            <div class="form-floating">
                                                <input type="date" name="date_agenda" class="form-control" id="" value="<?php echo $_SESSION['agenda']['date_agenda'];?>">
                                                 <label class="form-label">Date de la rencontre</label>                      
                                            </div>                                         
                                        </div>
                                        <div class='col-12 mb-1'>
                                            <div class="form-floating">
                                                <input type="time" name="heure_agenda" class="form-control" id="" value="<?php echo $_SESSION['agenda']['heure_agenda'];?>">
                                                 <label class="form-label">Heure de la rencontre</label>                      
                                            </div>                                         
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="card card-fluid mb-4" id='divPrixagenda'>
                                    <div class="card-header p-3 text-uppercase">  
                                        conférencié
                                    </div>
                                    <div class="card-body p-2">
                                        <div class="row">
                                            ...
                                            <div class='col-12 mb-1'>
                                                <div class="form-floating">
                                                    <input type="text" name="nom_conferencie" class="form-control" value="<?php echo $_SESSION['agenda']['nom_conferencie'];?>">
                                            ...
                                            <div class='col-12 mb-1'>
                                                <div class="form-floating">
                                                    <input type="text" name="fonction_conferencie" class="form-control" value="<?php echo $_SESSION['agenda']['fonction_conferencie'];?>">
                                            ...
                                            <div class='col-12 mb-1'>
                                                <div class="form-floating">
                                                    <div>
                                                        ...
                                                        <?php if (!empty($_SESSION['agenda']['image_conferencie'])): ?>
                                                            <div class="mt-3">
                                                                <img src="../fichiers/uploads/<?php echo $_SESSION['agenda']['image_conferencie']; ?>" alt="Image actuelle" class="img-fluid rounded" style="max-width: 50px;">
                                                            </div>
                                                        <?php endif; ?>
                                                    </div>
                          ...
                          <div class='col-md-4'>
                              ...
                              <div class="card card-fluid mb-4">
                                <div class="card-header p-3 d-flex justify-content-between align-items-center">
                                    <span>Image agenda</span>
                            <div class="d-flex">
                            <label for="fileInput" class="btn btn-link py-0 px-2 text-body fs-9"> <span class="fa-solid fa-plus"></span></label>
                            <input id="fileInput" type="file" class="d-none" onchange="uploadImgagenda(this)" accept="image/*" />
                            </div>
                                </div>
                                <div class="card-body p-5 mx-auto">
                                    <div id='divImgPrincipale'><?php echo affImgAdmin('200px', '200px', $_SESSION['agenda']['image'], '');?></div>
                                </div>
                              </div>   
                              ...
                              <div class="card card-fluid mb-4">
                                ...
                                <div class="card-body">
                                    <div>
                                        <?php
                                        $query = "SELECT id FROM agenda WHERE id = ".$_SESSION['agenda']['id']." LIMIT 1";
                                        $resultat = mysqli_query($bdd, $query) or die("Requête non conforme");  
                                        $select_cats_agenda = mysqli_fetch_array($resultat); 

                                        if(!empty($select_cats_agenda)){$checked = 'checked';}else{$checked = '';}
                                        ?>
                                        ...
                                        <?php
                                    // Étape 1 : récupérer l'id de la catégorie actuelle de l'agenda en cours
                                    $id_categorie_actuelle = 0;
                                    $query_cat = "SELECT id_cat_agenda FROM agenda WHERE id = ".$_SESSION['agenda']['id']." LIMIT 1";
                                    $result_cat = mysqli_query($bdd, $query_cat);
                                    if ($row = mysqli_fetch_assoc($result_cat)) {
                                        $id_categorie_actuelle = $row['id_cat_agenda'];
                                    }
                              ...
                              <div class="card card-fluid mb-4">
                                <div class="card-header p-3 d-flex justify-content-between align-items-center">
                                    ...
                                    <input id="fileInput2" type="file" class="d-none" onchange="uploadGalerieagenda(this)" accept="image/*" multiple/>
                                    </div>          
                                </div>
                                <div class="card-body text-center p-0" id='tbodyGalerieagenda'>
                              ...
                              <div class="card card-fluid mb-4">
                                <div class="card-header p-3">
                                    <h4 class="text-body">Statut événement</h4> 
                                </div>
                                <div class="card-body p-2">
                                    <div id='divCheckedStatutagenda'>
                                        <div class="form-check">
                                            <input type="radio" name="statut_agenda" class="form-check-input" 
                                                <?php echo comp_2_val_retour($_SESSION['agenda']['statut'], 'supprimé', 'checked');?> 
                                                id="statutagendaSupprime" value='supprimé'>
                                            <label class="form-check-label text-danger" for="statutagendaSupprime">Supprimé</label>
                                        </div>
                                        <div class="form-check">
                                            <input type="radio" name="statut_agenda" class="form-check-input" 
                                                <?php echo comp_2_val_retour($_SESSION['agenda']['statut'], 'en cours', 'checked');?> 
                                                id="flexRadioDefault3" value='en cours'>
                                            <label class="form-check-label" for="flexRadioDefault3">En cours</label>
                                        </div>
                                        <div class="form-check">
                                            <input type="radio" name="statut_agenda" class="form-check-input" 
                                                <?php echo comp_2_val_retour($_SESSION['agenda']['statut'], 'terminé', 'checked');?> 
                                                id="flexRadioDefault4" value='terminé'>
                                            <label class="form-check-label" for="flexRadioDefault4">Terminé</label>
                                        </div>
                                    </div>
                                    <input type="hidden" id="id_agenda_statut" value="<?php echo crypt_decrypt_chaine($_SESSION['agenda']['id'], 'C');?>">
                      ...
                      <div>
                        ...
                        <input type='hidden' name='id_agenda' value="<?php echo crypt_decrypt_chaine($_SESSION['agenda']['id'], 'C');?>"> 

                        <input type='hidden' name='actualiserInfosagenda'>
    ...
    <script>
        $(document).ready(function() {
            $('#editor').summernote({
                ...
                callbacks: {
                    onInit: function() {
                        let aproposContent = `<?php echo !empty($_SESSION['agenda']['description']) ? htmlspecialchars_decode($_SESSION['agenda']['description']) : ''; ?>`;
...
function uploadImgagenda(element)
{
      ...
      img.onload = function() {
          ...
          $.ajax({
              ...
              data: { 
                  ...
                  id_agenda: "<?php echo crypt_decrypt_chaine($_SESSION['agenda']['id'], 'C');?>",    
                  uploadImagePrincipaleagenda: ''
              },
...
function listeGalerieagenda()
{
	$.ajax({
		...
		success: function(response) {
			...
			$('#tbodyGalerieagenda').html(response); 
		}
	});
}
...
function uploadGalerieagenda(element)
{ 
...
function supprGalerieagenda(element)
{
...
function actualiserStatut() {
    try {
        ...
        var radio = document.querySelector('input[name="statut_agenda"]:checked');
        if (!radio) {
        ...
        var idAgendaInput = document.getElementById('id_agenda_statut');
        if (!idAgendaInput) {
            console.log("Champ #id_agenda_statut introuvable.");
            return;
        }

        var idAgenda = idAgendaInput.value;

        console.log('Données envoyées : statut=' + statut + ', id=' + idAgenda);
        ...
        $.ajax({
            ...
            type: 'POST',
            data: { 
                id_agenda_statut: idAgenda,
                statut_agenda: statut,
                actualiserStatut: ''
            },
            dataType: 'html',
            success: function(response) {
